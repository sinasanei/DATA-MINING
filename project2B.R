...
setwd("/Users/BSS/Documents/5. 2018.1 Spring/Stat 557/project2/good")
...
for(i in 1:10){
  ...
  x4 = matrix(NA,dimv,dimh)
  x5 = matrix(NA,dimv,dimh)
  
  x4 = rep(round((1:dimv)/10),dimh)
  x5 = rep(round((1:dimh)/10),dimv)
  
  nmx_4[[i]] = matrix(x4,dimv,dimh)
  nmx_5[[i]] = t(matrix(x5,dimh,dimv))
  
  vmx_4[[i]] = matrix(nmx_4[[i]],dimv*dimh)
  vmx_5[[i]] = matrix(nmx_5[[i]],dimv*dimh)
  print(i)
}

#save(nmx_1,nmx_2,nmx_3,nmx_4,nmx_5, file="nmx.Rd")
#save(vmx_1,vmx_2,vmx_3,vmx_4,vmx_5, file="vmx.Rd")
load("nmx.Rd")
load("vmx.Rd")
...
for(i in c(1,2,3,5,6,7,8,9)){
  ...
  train.x.4=c(train.x.4,vmx_4[[i]])
  train.x.5=c(train.x.5,vmx_5[[i]])
...
#-------------------------------------------------------------
dat1 = data.frame(y = factor(train.y.1), x0 = train.x, x1 = train.x.1, x2 = train.x.2, x3 = train.x.3 ) #, x4 = train.x.4, x5 = train.x.5 )
dat2 = data.frame(y = factor(train.y.2), x0 = train.x, x1 = train.x.1, x2 = train.x.2, x3 = train.x.3 ) #, x4 = train.x.4, x5 = train.x.5 )
dat3 = data.frame(y = factor(train.y.3), x0 = train.x, x1 = train.x.1, x2 = train.x.2, x3 = train.x.3 ) #, x4 = train.x.4, x5 = train.x.5 )
dat.y = data.frame(y = (1)*train.y.1+(2)*train.y.2+(3)*train.y.3)
dat.x = data.frame(x0 = train.x, x1 = train.x.1, x2 = train.x.2, x3 = train.x.3 ) #, x4 = train.x.4, x5 = train.x.5 )
...
k = 1 # index of sample
km.vmy = km$cluster[1:length(vmy[[k]])]
km.nmy = matrix(km.vmy,dim(nmy[[k]])[1],dim(nmy[[k]])[2])

par(mfrow=c(1,2))
plot.img(km.nmy-1,s=5,c=T)
#--------------------------------------------------------
temp = matrix(vmy[[k]],dim(nmy[[k]])[1],dim(nmy[[k]])[2])
plot.img(my1,s=50,c=T)
#plot.img(temp,s=5,c=T)
#plot(x=c(0,1,2),y=c(1,1,1),col=c(0,1,2),pch=19)
#--------------------------------------------------------
...
for(M in 3:20){
  km[[M]] = kmeans(dat.x,M)
  km.new[[M]] = km[[M]]$cluster

  check = km.new[[M]]*10+dat.y
  new.cls = rep(NA,M)
  for(m in 1:M){
    new.cls[m] = which.max(table(check[which(round(check/10)==m),]))
    km.new[[M]][which(km[[M]]$cluster==m)] = new.cls[m]
  }
  
  error[M] = sum(km.new[[M]]!=dat.y)/length(dat.y[,1])
  total.k[M] = length(unique(km.new[[M]]))
  print(M)
}
...
k = 1     # index of sample
i = 15    # index of model (number of prototype)
km.vmy = km[[i]]$cluster[1:length(vmy[[k]])]
km.vmy = km.new[[i]][1:length(vmy[[k]])]
km.nmy = matrix(km.vmy,dim(nmy[[k]])[1],dim(nmy[[k]])[2])
