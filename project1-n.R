...
##############################################################
# check the image
plot.img <- function(samp.x, s=3, col=F){
  s=3  # definition of image
  #samp.x = my                 #image x
...
table(my)
save(my_1,my_2,my_3, file="nmy.Rd")
save(vmy_1,vmy_2,vmy_3, file="vmy.Rd")
...
yglm_1=predict(fitglm2_1, newdata=newdat, type='response')
yglm_2=predict(fitglm2_2, newdata=newdat, type='response')
yglm_3=predict(fitglm2_3, newdata=newdat, type='response')
