...
table(yglm)
table(nmy[[2]])
my = matrix(yglm,dim(nmx[[2]])[1],dim(nmx[[2]])[2])
error = sum(yglm!=nmy[[2]])/length(nmy[[2]])

plot.img(my,s=5,col=T)
plot.img(nmy[[2]],col=T)
plot.img(nmx[[2]])
...
fitglm_1=glm(y~x0+x1+x2, data=dat1, family='binomial')
fitglm_2=glm(y~x0+x1+x2, data=dat2, family='binomial')
fitglm_3=glm(y~x0+x1+x2, data=dat3, family='binomial')
summary(fitglm_1)

newdat = data.frame(x0 = vmx[[2]], x1 = vmx_1[[2]], x2 = vmx_2[[2]], x3 = vmx_3[[2]])
yglm_1=predict(fitglm_1, newdata=newdat, type='response')
yglm_2=predict(fitglm_2, newdata=newdat, type='response')
yglm_3=predict(fitglm_3, newdata=newdat, type='response')
...
table(ylda)
table(nmy[[2]])
my = matrix(ylda,dim(nmx[[2]])[1],dim(nmx[[2]])[2])
error = sum(ylda!=nmy[[2]])/length(nmy[[2]])
...
table(yqda)
table(nmy[[2]])
my = matrix(yqda,dim(nmx[[2]])[1],dim(nmx[[2]])[2])
error = sum(yqda!=nmy[[2]])/length(nmy[[2]])
